...
namespace Util.Redis
{
    ...
    public class OperatorHelper
    {
        ...
        public OperatorHelper(int dbNum = 0)
            : this(dbNum, null)
        {
        }

        public OperatorHelper(int dbNum, string readWriteHosts)
        {
            DbNum = dbNum;
            _conn =
                string.IsNullOrWhiteSpace(readWriteHosts)
                    ? ConnectionHelper.Instance
                    : ConnectionHelper.GetConnectionMultiplexer(readWriteHosts);
        }
        ...
        {
            var prefixKey = CustomKey ?? ConnectionHelper.SysCustomKey;
        ...
        public double IncreaceString(string key, double val = 1)
        {
            key = AddSysCustomKey(key);
        ...
        public double DecreaseString(string key, double val = 1)
        {
            key = AddSysCustomKey(key);
        ...
        /// <param name="key">Redis Key</param>
        /// <returns></returns>
        public async Task<string> SetStringAsync(string key)
        {
            key = AddSysCustomKey(key);
        ...
        /// <param name="key"></param>
        /// <param name="value"></param>
        public void PushRightList<T>(string key, T value)
        {
            key = AddSysCustomKey(key);
        ...
        /// 删除单个key
        /// </summary>
        /// <param name="key">redis key</param>http://time.qq.com/?pgv_ref=aiotime
